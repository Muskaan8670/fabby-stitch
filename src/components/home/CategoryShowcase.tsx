...
export default function CategoryShowcase() {
  ...
  const secondaryCategories = CATEGORIES.filter((c) => !c.featured);
  ...
  return (
    <section className="w-full bg-[#FAF9F6] text-[#121212] py-20 sm:py-28 lg:py-36 px-4 sm:px-6 lg:px-8 border-b border-[#E4E4E7]">
      <div className="max-w-7xl mx-auto space-y-12 sm:space-y-16">
        ...
        {/* Editorial Multi-Column Category Grid */}
        <div className="space-y-8">
          
          {/* Top Row: 2 Major Featured Categories (Tees & Hoodies) */}
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6 lg:gap-8">
            {featuredCategories.map((category) => (
              <Link
                key={category.id}
                href={category.href}
                className="group relative block w-full aspect-[4/5] sm:aspect-[4/3] md:aspect-[4/5] bg-zinc-900 overflow-hidden border border-[#E4E4E7]"
              >
          ...
          {/* Bottom Row: 3 Secondary Categories (Polos, Jackets, Caps) */}
          <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-6 lg:gap-8">
            {secondaryCategories.map((category) => (
              <Link
                key={category.id}
                href={category.href}
                className="group relative block w-full aspect-[4/5] bg-zinc-900 overflow-hidden border border-[#E4E4E7]"
              >
                {/* Image */}
                <Image
                  src={category.imageSrc}
                ...
                <div className="absolute inset-0 p-6 flex flex-col justify-between z-10">
                  ...
                  <div className="space-y-1">
                    ...
                    <div className="flex items-center justify-between">
                      <h3 className="text-lg sm:text-xl font-light uppercase tracking-tight text-white group-hover:text-[#C5A059] transition-colors">
                        {category.name}
                      </h3>
                      <svg
                        className="w-4 h-4 text-white transform transition-transform duration-300 group-hover:translate-x-1 group-hover:text-[#C5A059]"
                        fill="none"
                        stroke="currentColor"
                        viewBox="0 0 24 24"
                      >
