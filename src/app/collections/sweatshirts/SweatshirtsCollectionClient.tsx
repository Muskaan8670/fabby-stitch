...
const CATEGORY_NAV_ITEMS = [
  ...
  { label: "SWEATSHIRTS", href: "/collections/sweatshirts", active: true },
...
export default function SweatshirtsCollectionClient() {
  // Filter State tailored specifically to Sweatshirts
  ...
  // Base Sweatshirt products pool from central dataset
  const sweatshirtProducts = useMemo(() => {
    return PRODUCTS.filter((p) => p.categorySlug === "sweatshirts");
  }, []);

  // Filter & Sort Logic
  const filteredProducts = useMemo(() => {
    return sweatshirtProducts
      .filter((p) => {
        // Availability
      ...
      <main className="flex-1">
        ...
        <nav aria-label="Breadcrumb" className="bg-white border-b border-zinc-200/80 py-3 px-4 sm:px-6 lg:px-8">
          <div className="max-w-7xl mx-auto flex items-center gap-2 text-[10px] sm:text-xs uppercase tracking-widest text-zinc-500 font-medium">
            ...
            <span>/</span>
            <span className="text-zinc-900 font-semibold">Sweat Shirts</span>
          </div>
        </nav>
        ...
        <section className="w-full bg-white border-b border-[#E4E4E7] py-12 sm:py-16 px-4 sm:px-6 lg:px-8">
          <div className="max-w-7xl mx-auto space-y-3">
            ...
            <h1 className="text-4xl sm:text-6xl font-light uppercase tracking-tight text-[#121212]">
              Sweat Shirts<span className="font-semibold">.</span>
            </h1>
            <p className="max-w-2xl text-xs sm:text-sm text-zinc-600 leading-relaxed font-normal">
              Explore classic Fabby Stitch soft fleece crewneck sweatshirts designed for layered streetwear comfort and direct workshop customization ($31.95).
            </p>
          </div>
        </section>
            ...
            <div className="flex-1 space-y-10">
              {filteredProducts.length === 0 ? (
                <div className="bg-white border border-zinc-200 p-12 text-center space-y-4 max-w-md mx-auto my-12">
                  <h3 className="text-lg font-semibold uppercase tracking-wider">NO SWEAT SHIRTS FOUND</h3>
                  <p className="text-xs text-zinc-500 leading-relaxed">
                    Try adjusting your filters to find available Sweat Shirts.
                  </p>
                  <button
        ...
        {/* 5. Custom Sweat Shirt CTA Banner */}
          ...
          <div className="max-w-7xl mx-auto flex flex-col sm:flex-row items-center justify-between gap-6">
            <div className="space-y-1 text-center sm:text-left">
              ...
              <h3 className="text-xl sm:text-2xl font-light uppercase tracking-tight text-white">
                MAKE IT <span className="font-semibold text-white">YOUR OWN.</span>
              </h3>
              <p className="text-xs text-zinc-400">
                Need custom Sweat Shirts for your brand, team, organization or project?
              </p>
            </div>
            <Link
              href="/custom-quote"
